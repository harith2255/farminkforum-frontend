  ...
  const checkAccess = async () => {
    try {
      const res = await fetch(
        `e-book-backend-production.up.railway.app/api/drm/check-access?note_id=${note.id}`,
        {
  ...
  (async () => {
    try {
      const hres = await fetch(
        `e-book-backend-production.up.railway.app/api/notes/highlights/${note.id}`,
      ...
      const pres = await fetch(
        `e-book-backend-production.up.railway.app/api/notes/lastpage/${note.id}`,
        { headers: authHeaders(token) }
      );
...
useEffect(() => {
 ...
 const handleAddHighlight = async (h: any) => {
  try {
    const res = await fetch(
      "e-book-backend-production.up.railway.app/api/notes/highlights",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleDeleteHighlight = async (id: number) => {
    try {
      await fetch(
        `e-book-backend-production.up.railway.app/api/notes/highlights/${id}`,
        {
          method: "DELETE",
headers: authHeaders(token),        }
      );
