...
const deleteNote = async (noteId: number) => {
  ...
  try {
    ...
    await axios.delete(
      `${import.meta.env.VITE_API_URL}/api/admin/exams/notes/${noteId}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
...
const deleteExam = async (examId: number) => {
  ...
  try {
    const token = tokenRef.current;
    await axios.delete(
      `${import.meta.env.VITE_API_URL}/api/admin/exams/exams/${examId}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
...
const sortedExams = useMemo(
  ...
  const loadSubjects = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/admin/exams/subjects`,
        {
          headers: { Authorization: `Bearer ${token}` },
          timeout: 10000,
        }
  ...
  const loadFolders = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/admin/exams/folders`,
        {
          headers: { Authorization: `Bearer ${token}` },
          timeout: 15000,
        }
  ...
  const fetchSubmissions = useCallback(async (examId: number) => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/admin/exams/${examId}/submissions`,
  ...
  const gradeSubmission = async (submissionId: number) => {
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/admin/exams/submissions/${submissionId}/grade`,
        payload,
        { headers: { Authorization: `Bearer ${token}` }, timeout: 10000 }
      );
  ...
  try {
    ...
    await axios.delete(
      `${import.meta.env.VITE_API_URL}/api/admin/exams/subject/${selectedSubject.id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
        timeout: 15000,
      }
...
const handleUpload = async () => {
  ...
  try {
    ...
    if (examPDF) {
      ...
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/admin/exams/${examId}/upload-file`,
        formExam,
        { headers: { Authorization: `Bearer ${token}` } }
      );
    }
  ...
  const saveEditExam = async () => {
    ...
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/admin/exams/${editExam.id}`,
        {
